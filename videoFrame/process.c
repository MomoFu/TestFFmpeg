...
AVFrame *readAVFrame (AVCodecContext *pCodecCtx, AVFormatContext *pFormatCtx,
    ...
    int64_t seek_target= av_rescale_q((double)timeStamp / 1000 * AV_TIME_BASE, AV_TIME_BASE_Q, pFormatCtx->streams[stream_index]->time_base);
    ...
    if (ret < 0) {
        fprintf(stderr, "av_seek_frame failed1");
        return NULL;
    }
